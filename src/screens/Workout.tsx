...
const Workout = () => {
  ...
  const [overallMinutes, setOverallMinutes] = useState<Activity[]>([]);
  ...
  useEffect(() => {
    ...
    if (
      (settings && settings.activitiesOneRound.length > 0) ||
      (settings && settings.overallTimeInSec > 0)
    ) {
      const combinedMinutes = getOverallMinutes(
        settings.activitiesOneRound,
        settings.overallTimeInSec
      );
  ...
  return (
    <Screen>
      ...
      <Timer
        ...
        countInSec={settings ? settings.overallTimeInSec : 0}
        playback={playback}
      ...
      {overallMinutes.map((minute, index) => {
        ...
        const labelNextMinute =
          nextMinuteAvailable && overallMinutes[index + 1].description;
        const restInSec =
          minute.type === ActivityType.ExcerciseForTime
            ? 60 - minute.durationInSec
            : 0;
        ...
        return (
          index === currentTimerIndex && (
            <div key={index}>
              {nextMinuteAvailable && (
                <label>Next: {overallMinutes[index + 1].description}</label>
              )}
              <Timer
                label={`min ${index + 1}: ${minute.description}`}
